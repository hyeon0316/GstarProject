...
public class SpawnProjectilesScript : MonoBehaviour {
	...
	public static SpawnProjectilesScript inst;
	...
	{
		/*
		if ((Input.GetKey(KeyCode.A)|| Input.GetMouseButton(0)) && Time.time >= timeToFire) {
			RaycastHit hit;
			...
			Player.transform.parent.GetComponent<Player>().isMove = false;
			Player.transform.parent.GetComponent<Player>().animator.SetBool("isMove", false);
			Player.transform.parent.GetComponent<Player>().animator.SetTrigger("attack");
	...
	public void SpawnVFX () {
		...
		if (firePoint != null) 
		{
			vfx = Instantiate (effectToSpawn, firePoint.transform.position, Quaternion.LookRotation(firePoint.transform.forward));
            if (!useTarget)
            {
				/*
