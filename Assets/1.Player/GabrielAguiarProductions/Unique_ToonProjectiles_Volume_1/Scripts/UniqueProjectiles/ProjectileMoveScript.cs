...
public class ProjectileMoveScript : MonoBehaviour {
	...
	void Start () {
		...
		lScale = new Vector3(0.3f, 0.3f, 0.3f);
		...
		if (muzzlePrefab != null) {
			...
			muzzleVFX.transform.localScale = lScale;
	...
	void FixedUpdate () {
		...
		if (speed != 0 && rb != null)
		{
			moveS = transform.forward + offset;
			rb.position += moveS * (speed * Time.deltaTime);
			rb.position = new Vector3(rb.position.x, startPos.y, rb.position.z);
		}
	...
	void hit(Collision co)
    {
        if (co.gameObject.tag != "Bullet" && !collided)
        {
            ...
            if (hitPrefab != null)
            {
                ...
                hitVFX.transform.localScale = new Vector3(0.3f, 0.3f, 0.3f);
                if (ps == null)
                {
