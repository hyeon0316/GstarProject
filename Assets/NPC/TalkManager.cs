...
public class TalkManager : MonoBehaviour
{
    ...
    void GenerateData()
    {
        ...
        talkData.Add(8000 + 10 + 0, new string[] { "너...너.. 뭐하는 녀석이야!!!!", "우리가 타야하는 텔레포트였다고!!!!", "후.. 일단 스승님한테 가봐.." });
        talkData.Add(7000 + 10 + 1, new string[] { "허허...신기한 일을 다 겪는구만..","일단.. 자네도 원래 있던곳으로 돌아가야 하지 않나",
            "우리는 지금 바뻐서 재료를 다시 모을수가 없네","자네가 대신 해줘야 할거같은데",
            "마력과 아이템을 조금 나눠주겠네.", "지금은 기본공격 밖에 할수없을거네.",
            "재료를 모아와주면 다른 스킬들을 주겠네.", "광산에가서 동굴거미의 심장을 5개 가져와 주게.." });
        ...
        talkData.Add(7000 + 30 + 0, new string[] { "소피아양 이제 내가 줄수있는건 다줬네", "던전 입구에 있는 기사단장 엔 에게 가보게","그녀가 도움을줄거야" });

        talkData.Add(9000 + 30 + 1, new string[] { "당신이 소피아양인가요?", "던전은 매우위험합니다 \n 사정은 들었습니다.", "어쩔 수 없네요 숙련자의 장비를 드리겠습니다.", "무운을 빕니다", "리치의 정보는 던전안의 유령이 알고 있을것 입니다." }) ;
        
        
        talkData.Add(4000 + 40 + 0, new string[] { "어서와! 어서와!","나? 나는 예전에 리치에게 \n도전했다가 패배해서 이렇게 변했지 머람~",
            "너가 촌장님이 말한 친구지?!","리치를 쓰러트려야 한다고?\n 그럼 내부탁을 들어줘 그럼 방법을 알려줄게!","던전 1번방에가서 고블린을잡고 광기 가득한눈 30개를 모아줘!"});
        talkData.Add(4000 + 40 + 1, new string[] { "아직 광기 가득한눈을 다 못구한거같은데?" });
        talkData.Add(4000 + 40 + 2, new string[] { "이야 벌써구해왔어? ","보상으로 포션이랑 경험치 조금 줄게!","다시 말 걸어줘!"});
        talkData.Add(4000 + 40 + 3, new string[] { "여기 보상이야!" });


        talkData.Add(4000 + 50 + 0, new string[] {"리치한테가는법이 궁금하다고헀지?",
            "리치의 방에 갈려면 보물방에 있는 \"마력 제어 목걸이\"가 있어야하는데","지금 보물방은 잠겨있어 너가 열쇠랑 재료를 구해와줘","보물방 열쇠 두개로 나눠져있어","던전 2,3번방에서 몬스터를 잡다보면 하나씩나올거야!","아 그리고 잡는김에 던전2,3번방 전리품도 챙겨와줘! 30개씩" });

        talkData.Add(4000 + 50 + 1, new string[] { "아직 다 못구한거같은데?" });
        talkData.Add(4000 + 50 + 2, new string[] { "열쇠조각을 다 구했구나! 다시 말을 걸어줘 금방 보물방을 열어줄게" });
        talkData.Add(4000 + 50 + 3, new string[] { "보물방이 열렸어!" });

        
        talkData.Add(4000 + 60 + 0, new string[] { "보물방은 던전1에서 앞으로 쭉가면 나올거야 ","보물방을 잘찾아보면 \"마력제어목걸이\"를 가져와줘!",
        ...
        talkData.Add(4000 + 70 + 1, new string[] { "리치를 쓰러뜨렸다고?... 맙소사..","고생했어 \"해골 물약\"을 마을 촌장한테 전해주면 촌장이 마력을 전부 회복할꺼야" });
        talkData.Add(7000 + 70 + 2, new string[] { "고생했네 소피아양","재료를 다모았네 잠시만 기다려주게.." });
